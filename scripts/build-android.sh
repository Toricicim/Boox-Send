...
case "$variant" in
    ...
    *) echo "Error: BOOX_SEND_ANDROID_VARIANT must be debug or release." >&2; exit 1 ;;
esac
...
test -f "$source_apk" || { echo "Error: APK was not created: $source_apk" >&2; exit 1; }
mkdir -p "$output_root"
cp "$source_apk" "$output_root/$output_name"
echo "Android package ready: $output_root/$output_name"
